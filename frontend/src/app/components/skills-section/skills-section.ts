      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
          @for (category of skillCategories(); track category.id) {
            <div class="space-y-6">
              <h3 class="text-zinc-500 text-xs font-bold uppercase tracking-widest">
                {{ ls.t()('SKILLS.' + category.id) }}
              </h3>
              <ul class="space-y-4">
                @for (skill of category.skills; track skill) {
                  <li class="flex items-center gap-3 group">
                    <div class="w-1.5 h-1.5 rounded-full bg-zinc-800 group-hover:bg-white transition-colors"></div>
                    <span class="text-zinc-300 group-hover:text-white transition-colors font-medium">{{ skill }}</span>
                  </li>
                }
              </ul>
            </div>
